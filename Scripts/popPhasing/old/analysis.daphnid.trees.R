...
### libraries
  ...
  njo <- read.tree("~/Scaffold_2217_HRSCAF_2652_5173221_5223221.fasta.treefile")


  njo <- root(njo, outgroup="March20_2018_DBunk_38.1.fa_Scaffold_2217_HRSCAF_2652_5173222-5223221")
  d <- data.table(label=njo$tip.label)
  d[,pond:=tstrsplit(label, "_")[[4]]]
  d[,A:=ifelse(grepl("April_20rea17_D8_213", label), "A", "")]
...
### iterate through
  foreach(i=1:length(mats))%do%{
    ...
    njo <- nj(dm)
    njo <- root(njo, outgroup="March20_2018_DBunk_38;Scaffold_2217_HRSCAF_2652:5173222-5223221")
    d <- data.table(label=njo$tip.label)
    d[,pond:=tstrsplit(label, "_")[[4]]]
    d[,A:=ifelse(grepl("April_20rea17_D8_213", label), "A", "")]
    d[,C:=ifelse(grepl("April_2017_D8_151", label), "C", "")]
