...
### load in F1 validation (`DaphniaPulex20162017Sequencing/AlanAnalysis/QTL_superclone_test/F1_mapping_validation.polarizeHapolotypes_poolSeq.R`)
  ...
  summary(t1 <- glm(propmale~nMale, f1.val, weights=N, family="binomial"))
  summary(t2 <- glm(propmale~1, f1.val, weights=N, family="binomial"))
  anova(t1, t2, test="Chisq")

  summary(t1 <- glm(propmale~nMale, f1.val[gr=="AxC"], weights=N, family="binomial"))$coef
  summary(t2 <- glm(propmale~1, f1.val[gr=="AxC"], weights=N, family="binomial"))$coef
  anova(t1, t2, test="Chisq")

  summary(t1<- glm(propmale~nMale, f1.val[gr=="CxC"], weights=N, family="binomial"))$coef
  summary(t2<- glm(propmale~1, f1.val[gr=="CxC"], weights=N, family="binomial"))$coef
  anova(t1, t2, test="Chisq")
...
#################
  ...
  ggsave(male.plot + epp.plot + f1polar.plot,
          file="DaphniaPulex20162017Sequencing/AlanFigures/Figure4/qtl_mega_toprow.pdf", width=8, height=2)
